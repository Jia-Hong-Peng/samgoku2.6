...
sub BUY2 {
	...
	if($in{'num'} eq ""){&ERR("沒有輸入買賣數。");}
	if($in{'type'} eq ""){&ERR("買賣沒有輸入。");}
	if($in{'num'} =~ m/[^0-9]/){&ERR("買賣數中含有數字以外的文字。"); }
	&CHARA_MAIN_OPEN;
	&TOWN_DATA_OPEN("$kpos");
	...
	if($in{'type'}){
		$title_name = "$in{'num'}買米";
	}else{
		$title_name = "$in{'num'}換金";
	}
	...
	print <<"EOM";
<CENTER><hr size=0><h2><font color="#FFFFFF">NO:$no輸入了$title_name。</font></h2><p>
