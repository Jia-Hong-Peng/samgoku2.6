...
sub MOVE2 {
	...
	if($in{'no'} eq "all"){
		while(@NEW_COM_DATA < $MAX_COM){
				push(@NEW_COM_DATA,"$in{'mode'}<><>$town_name[$num] ²¾°Ê<>$tt<><>$in{'num'}<><>\n");
		}
		$no = $in{'no'};
	}else{
		foreach(@COM_DATA){
	...
	print <<"EOM";
<CENTER><hr size=0><h2><font color="#FFFFFF">NO:$no‚É$town_name[$num]²¾°Ê¿é¤J¡C</font></h2><p>
