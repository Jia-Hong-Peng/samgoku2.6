...
&HOST_NAME;
	...
	foreach (@GUEST){($timer,$name,$con,$opos,$rkhost) = split(/<>/);
$muti = "";
foreach (@GUEST){($timer1,$name1,$con1,$opos1,$rkhost1) = split(/<>/);
if( $times - 10 > $timer){
			next;
		}
elsif ($rkhost eq $rkhost1)		
{
if ($name ne $name1)
{
$muti .= "<font color=red>[$name1]</font>";
}
}	
}
if ($muti ne "")
{
	...
	unshift(@S_MOVE,"<font color=red><B>\[¦h­«\]</B></font> $name¦@¾Ö¦³¥H¤U±b¸¹¡G$muti¡C($mday¤é$hour®É$min¤À)\n");
	splice(@S_MOVE,20);

	open(OUT,">$MUTI_LOG_LIST") or &ERR2('LOG ¤£¯à¼g¤W·sªº¼Æ¾Ú¡C');
	print OUT @S_MOVE;
	close(OUT);
}
		if( $times - 180 > $timer){
			...
			push (@New_guest_list,"$times<>$kname<>$kcon<>$kpos<>$rkhost<>\n");
$m_list .= "$kname\[$town_name[$kpos]\]$muti ";
			$hit = 1;
		}else{
			push (@New_guest_list,"$timer<>$name<>$con<>$opos<>$rkhost<>\n");
			if($kcon eq $con){
				$m_list .= "$name\[$town_name[$opos]\]$muti";
			}
		}
	}

	if(!$hit){
		push(@New_guest_list,"$times<>$kname<>$kcon<>$kpos<>$host<>\n");
		$m_list .= "$kname\[$town_name[$kpos]\]$muti ";
	}
